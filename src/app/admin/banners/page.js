...
export default function BannerManagement() {
  ...
  const fetchBanners = async () => {
    ...
    try {
      ...
      if (!error && data && data.length > 0) {
        setBanners(data);
        localStorage.setItem('argun_banners', JSON.stringify(data));
      } else {
        ...
        if (local) {
          ...
          localStorage.setItem('argun_banners', JSON.stringify(DEFAULT_BANNERS));
        }
      }
    } catch (err) {
      const local = localStorage.getItem('argun_banners');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editBanner) {
        ...
        setBanners(updated);
        localStorage.setItem('argun_banners', JSON.stringify(updated));
      } else {
      ...
      let updated = [];
      if (editBanner) {
        updated = banners.map(b => String(b.id) === String(editBanner.id) ? { ...b, ...payload } : b).sort((a,b) => a.display_order - b.display_order);
      } else {
  ...
  // Drag and drop image handler (with smart automatic compression to fit within localStorage)
  const handleImageUpload = (e, field) => {
    ...
    if (file) {
      ...
      img.onload = () => {
        ...
        // Max dimensions for high quality but lightweight images
        const MAX_WIDTH = field === 'mobile_image' ? 600 : 1200;
        const MAX_HEIGHT = field === 'mobile_image' ? 800 : 800;
        let width = img.width;
        ...
        // Convert to highly optimized JPEG (0.7 quality) to keep size under ~40KB
          ...
          >
            <div className="flex items-center justify-between p-6 border-b border-gray-100 bg-gray-50/50">
              <div>
                ...
                <p className="text-xs text-gray-500 mt-0.5">Customize options and preview live slide rendering instantly.</p>
              </div>
            ...
            <div className="p-6 border-t border-gray-100 bg-white flex justify-end gap-4 shadow-[0_-10px_20px_rgba(0,0,0,0.02)]">
              ...
              <button type="submit" disabled={isLoading} className="px-8 py-3 bg-[#1f163b] hover:bg-[#d4a54c] text-white rounded-xl text-xs font-bold uppercase tracking-widest transition-colors shadow-lg disabled:opacity-50 flex items-center gap-2">
